...
Solves Problem 1(b) and Problem 2 of the Linked List Practicum Assignment.

Structure
---------
Each digit of the integer is stored in a Python list element.
The list is ordered from the least-significant digit (index 0) to the
most-significant digit (last index), matching the conceptual structure of
the linked-list version.

Example:  45839  →  _digits = [9, 3, 8, 5, 4]
"""


class BigInteger:
    """
    Big Integer ADT implemented with a Python list.

    The list ``_digits`` stores individual decimal digits from
    least-significant (index 0) to most-significant (last index).
    Negative numbers are tracked with a boolean flag ``_negative``.
    ...
    def _parse(self, s: str) -> None:
        """Parse a decimal string and populate the digit list."""
        s = s.strip()
        if not s:
            s = "0"
        ...
        s = s.lstrip('0') or '0'
        if s == '0':
            self._negative = False  # normalise –0 → 0

        # Store digits: least-significant first (reverse of the string)
        self._digits = [int(ch) for ch in reversed(s)]

    # ── Private helpers ───────────────────────────────────────────────────

    def _to_int(self) -> int:
        """Convert the digit list to a plain Python int."""
        result = sum(digit * (10 ** i) for i, digit in enumerate(self._digits))
        return -result if self._negative else result

    def _update_from_int(self, value: int) -> None:
        """Overwrite this object's digit list from a plain Python int."""
    ...
    def toString(self) -> str:
        ...
        if not self._digits:
            return "0"
        # digits list is least-sig first, so reverse for display
        number = ''.join(str(d) for d in reversed(self._digits))
    ...
    def _apply_inplace(self, result: "BigInteger") -> "BigInteger":
        """Helper: overwrite self with result's digit list and return self."""
...
if __name__ == "__main__":
    print("=" * 60)
    print("Big Integer ADT – Python List Demo")
    print("=" * 60)
    ...
    print(f"\na = {a}   (digits list: {a._digits})")
    print(f"b = {b}   (digits list: {b._digits})")
    print(f"c = {c}  (digits list: {c._digits})")
